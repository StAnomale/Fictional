...
// ItemEvents.rightClicked('kubejs:swallowtail_butterfly', event => { //测试
...
//   if (event.player.cooldowns.isOnCooldown('kubejs:swallowtail_butterfly')) return;
...
//   player.addItemCooldown('kubejs:swallowtail_butterfly', 50/setCOOLDOWNS)
//   player.playNotifySound("minecraft:item.trident.hit", "ambient", 1, 1)
...
//   BladeBeam.spawn(player, 1 + 0.4 * damage + 0.1 * adddamage).distance(24).color(0xFF0000).alpha(0.2).build();
//   player.playNotifySound("minecraft:item.axe.strip", "ambient", 1, 1)
//   })
...
//     const damage = player.getAttributeTotalValue('minecraft:generic.max_health');
//     const adddamage = player.getAttributeTotalValue('obscure_api:magic_damage');
//     if(!pData.contains('swallowtail_butterfly')) return;
//     let tick = pData.getInt('swallowtail_butterfly');
//     if(tick > 0) pData.putInt('swallowtail_butterfly', --tick);
//     let level = event.getLevel();
//     let pos = player.position();
//     let pos1 = pos.add(new Vec3d(-4,-4,-4));
//     let pos2 = pos.add(new Vec3d(4,4,4));
...
//             entity.attack(player.damageSources().playerAttack(player), 1 + 0.4 * damage + 0.1 * adddamage);
//         });
//     player.potionEffects.add('slow_falling', 20, 0, false, false);
//     if(tick == 0) pData.remove('swallowtail_butterfly');
// })
